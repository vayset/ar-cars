...
function asset(value) {
  ...
  if (!/^\/(images|fonts|_next\/static)\//.test(u.pathname) && u.pathname !== '/favicon.svg') throw Error('Unexpected asset '+u.pathname);
...
for (const route of routes) {
  const $ = load(await (await read(origin+route)).text());
...
for (const [pathname] of pendingAssets) {
  const response=await read(origin+pathname);
